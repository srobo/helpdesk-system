    ...
    def _assign_free_users(self) -> None:
        for user in User.objects.all():
            if user.active_tickets().count() == 0:
                ...
                ticket = user.default_ticket_queue.active_tickets().filter(assignee__isnull=True).first()
                if ticket:
                    ticket.assignee = user
                    ticket.save()

    def _ticket_actions(self) -> None:
        for ticket in Ticket.objects.filter(assignee__isnull=False, resolved_at__isnull=True):
